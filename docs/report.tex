...
\begin{document}
	...
	\begin{itemize}
		\item Command line parsing: a \texttt{cv::CommandLineParser} is created. If a path to a valid dataset was not provided as a command line argument, the default relative path \texttt{../Final_project_proposal/} is used.
		\item Validation of the dataset path: the main function check that the data path is valid, i.e. non-empty and containg the subdirectories "test_photos", "train_healthy_photos" and "train_diseased_photos".
		...
		\item Results directory creation: the main function creates the output directory where the results of each classifier will be stored, if it does not exist yet.
	\end{itemize}
	...
	The Template Matching methods uses hand-picked template images (5 out of the 15 train healthy images available for every flower class) to classify the test images using the standard \texttt{cv::matchTemplate} function.
	...
	\subsection{Code Structure Decision}
	Template Matching logis is split into:
	\begin{itemize}
		\item \texttt{processImage} function: compares one test images with one set of templates (of one the classes)
		\item \texttt{template_match} function: complete wrapper of the Template Matching routine, iterates over all test images and compares similarity scores achieved with different classes to assign a final prediction
	\end{itemize}

	\subsection{Feature Extraction Choices}
	Since the Template Matching methods is heavily dependent on the size of both the template and the image been processed, and since templates images within each class have different sizes, all template images and associated masks are resize, with \texttt{cv::resize}, to a size of 400x300.
	...
	The \texttt{cv::matchTemplate} function allows a mask to be specified only if either TM_SQDIFF or TM_CCORR_NORMED are used as similarity scores.
	...
	\subsection{Robustness Choices}
	\begin{itemize}
		\item Empty input image check in \texttt{template_match(...)}.
	...
	\midrule
	\textbf{Total} & \textbf{$\sim$40} & \textbf{XX} & \textbf{$\sim$20} \\
	\bottomrule
	\end{tabular}
