...
\begin{document}
	...
	\subsection{Feature Extraction Choices}
	\begin{itemize}
		\item Input images are converted to grayscale, consistent with SURF's original design 
	...
	\subsection{Performance Considerations}
	SURF inherits similar trade-offs to SIFT:
	\begin{itemize}
		\item Memory usage scales linearly with training set size.
		\item Exhaustive matching against all training descriptors.
		\item No spatial consistency verification.
	\end{itemize}

	However, SURF typically produces fewer keypoints per image than SIFT (especially with higher 
	Hessian thresholds), which can reduce memory footprint and matching time. This is reflected 
	in the observed performance metrics, where SURF runs significantly faster than SIFT while 
